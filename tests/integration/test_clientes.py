...
def test_cadastrar_cliente_cpf_invalido(client, headers_admin):
    resposta = client.post("/clientes", json={
        "nome": "João", "email": "j@j.com", "telefone": "11",
        "cpf": "111.111.111-11",
    }, headers=headers_admin)
...
def test_cpf_duplicado_retorna_409(client, headers_admin):
    dados = {"nome": "João", "email": "j@j.com", "telefone": "11", "cpf": "529.982.247-25"}
...
def test_listar_clientes(client, headers_admin):
    client.post("/clientes", json={
        "nome": "Maria", "email": "m@m.com", "telefone": "11", "cpf": "529.982.247-25",
    }, headers=headers_admin)
...
def test_buscar_cliente_por_id(client, headers_admin):
    criado = client.post("/clientes", json={
        "nome": "Maria", "email": "m@m.com", "telefone": "11", "cpf": "529.982.247-25",
...
def test_atualizar_cliente(client, headers_admin):
    ...
    resposta = client.put(f"/clientes/{criado['id']}",
                          json={"nome": "Maria Silva"}, headers=headers_admin)
    assert resposta.status_code == 200
